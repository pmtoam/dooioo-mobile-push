...
public class PushHttpClient
{
	...
	private PushHttpResponse createPushHttpResponse(HttpResponse httpResponse)
			throws IllegalStateException, IOException
	{
		...
		pushHttpResponse.setStatusCode(httpResponse.getStatusLine()
				.getStatusCode());
		pushHttpResponse.setStatusText(httpResponse.getStatusLine()
				.getReasonPhrase());
			...
			HttpResponseHandler<T> responseHandler)
	{
		...
		PushSigner.sign(pushHttpRequest.getOriginalRequest(), pushHttpRequest,
				credentials);
		log.trace("after sign");

		log.info("httpRequest:" + pushHttpRequest);
		HttpRequestBase httpRequest = this.httpRequestFactory
				.createHttpRequestBase(pushHttpRequest);
		...
		try
		{
			...
			{
				throw this.handleErrorHttpResponse(httpResponse,
						this.errorResponseHandler).getResult();
			}
		}
	...
	private PushResponse<PushServiceException> handleErrorHttpResponse(
			HttpResponse httpResponse,
			ErrorResponseHandler<PushServiceException> errorResponseHandler)
			throws IllegalStateException, IOException
	{
		PushHttpResponse pushHttpResponse = this
				.createPushHttpResponse(httpResponse);
		return errorResponseHandler.handle(pushHttpResponse);
	}

	private <T> PushResponse<T> handleHttpResponse(HttpResponse httpResponse,
			HttpResponseHandler<T> responseHandler)
			throws IllegalStateException, IOException
	{
		PushHttpResponse pushHttpResponse = this
